...
//import 'package:google_maps_flutter_platform_interface/google_maps_flutter_platform_interface.dart' hide LatLng, Marker, CameraPosition, LatLngBounds, MarkerId;
import 'cluster.dart';
import 'cluster_item.dart';

class ClusterManager<T> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<pm.Marker> Function(Cluster<T>)? markerBuilder,
  ...
  final Future<pm.Marker> Function(Cluster<T>) markerBuilder;

  /// Function to update Markers on Google Map
  final void Function(Set<pm.Marker>) updateMarkers;
  ...
  pm.PlatformMapController? _mapController;
  ...
  void setMapController(pm.PlatformMapController controller,
  ...
  void onCameraMove(pm.CameraPosition position, {forceUpdate = false}) {
    _zoom = position.zoom;
    if (forceUpdate) {
      updateMap();
    }
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    final pm.LatLngBounds mapBounds = await _mapController!.getVisibleRegion();
    final pm.LatLngBounds inflatedBounds = _inflateBounds(mapBounds);
  ...
  pm.LatLngBounds _inflateBounds(pm.LatLngBounds bounds) {
  ...
  static Future<pm.Marker> Function(Cluster) get _basicMarkerBuilder =>
      (cluster) async {
        return pm.Marker(
          markerId: pm.MarkerId(cluster.getId()),
          position: cluster.location as pm.LatLng,
          onTap: () {
            print(cluster);
          },
  ...
  static Future<pm.BitmapDescriptor> _getBasicClusterBitmap(int size,
      {String? text}) async {
    ...
    return pm.BitmapDescriptor.fromBytes(data.buffer.asUint8List());
  }
}
